...
const chaincodeName = envOrDefault('CHAINCODE_NAME', 'vedalink');
...
async function createAsset(contract) {
    ...
    await contract.submitTransaction('CreateAsset', JSON.stringify(metaData));
...
async function transferAssetAsync(contract) {
    ...
    const newOwner = "newOwnerName"; //TODO

    const commit = await contract.submitAsync('TransferAsset', {
        arguments: [String(batchID), String(newOwner)],
    });
    const {asset,oldOwner} = utf8Decoder.decode(commit.getResult());

    console.log(
        `*** Successfully submitted transaction to transfer ownership of ${asset} from ${oldOwner} to ${newOwner}`
    );
